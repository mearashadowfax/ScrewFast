...
Marketing site locale: **en** only on this branch (`main` also ships **fr**). Each route file under `src/pages/` is a one-line shell around a shared view in `src/views/` that takes the locale as a prop; the locale/copy modules are kept so a second language is a small addition. Everything locale-related lives in [`src/utils/locale.ts`](src/utils/locale.ts); the current locale and its copy table are on `Astro.locals.locale` / `Astro.locals.copy` (set by [`src/middleware.ts`](src/middleware.ts)). Never parse `/fr` out of a pathname or hard-code a locale prefix elsewhere.
...
| Pages                   | [src/pages/](src/pages/)           | Astro file-based routing. Each file renders a view for its locale; `[id]` routes get `getStaticPaths` from `staticPathsFor()` in `@utils/content`.                                                                                                                                                                                            |
| Copy tables             | [src/copy/](src/copy/)             | `en.ts` defines the shape (`Copy`); any added locale file must match it or `astro check` fails. Every UI and page string lives here. Read via `Astro.locals.copy` or `getCopy(locale)`.                                                                                                                                                       |
| Content (collections)   | [src/content/](src/content/)       | `blog/`, `products/`, `insights/` with an `en/` subfolder; `docs/` for Starlight. Use `entriesFor()`, `pathFor()`, `localeOf()` from [`src/utils/content.ts`](src/utils/content.ts), never parse `entry.id`.                                                                                                                                  |
...
- **404**: `NotFoundView` is built at `/404`.
...
- **Copy:** no user-facing strings in components or views; add a key to `src/copy/en.ts`. Structure (paths, ids, URLs) lives in `src/data_files/`, labels in copy keyed by id; keys, not translated labels, drive logic (e.g. `badge: 'hiring'`).
...
- `pnpm test:smoke` – serve `dist/` and check every marketing route ([scripts/smoke.mjs](scripts/smoke.mjs))

## Recommendations for AI

- Use **path aliases** (`@components`, `@data`, `@images`, `@views`, etc.) in suggested code.
- To change a page, edit its view in [src/views/](src/views/) and its strings in [src/copy/](src/copy/).
- To add a locale: extend `MARKETING_LOCALES` and `LOCALE_INFO` in `locale.ts`, add `src/copy/<locale>.ts`, add `src/pages/<locale>/` shells, add content under `src/content/*/<locale>/` and JSON under `src/data_files/<locale>/`, and put `<LanguagePicker />` back in the navbars.
