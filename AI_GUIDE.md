...
| Pages                   | [src/pages/](src/pages/)           | Astro file-based routing; `fr/`, `de/` … mirror it. Each file renders a view for its locale; `[id]` routes get `getStaticPaths` from `staticPathsFor()` in `@utils/content`.                                                                                                                                                                                         |
| Copy tables             | [src/copy/](src/copy/)             | `en.ts` defines the shape (`Copy`); every other `<locale>.ts` must match it or `astro check` fails. Every UI and page string lives here. Read via `Astro.locals.copy` or `getCopy(locale)`.                                                                                                                                                                          |
| Content (collections)   | [src/content/](src/content/)       | `blog/`, `products/`, `insights/` with one subfolder per locale (`en/`, `fr/`, `de/`, `es/`, `fa/`, `ja/`, `zh-cn/`); `docs/` for Starlight. Use `entriesFor()`, `pathFor()`, `localeOf()` from [`src/utils/content.ts`](src/utils/content.ts), never parse `entry.id`.                                                                                              |
...
- **404**: `NotFoundView` is built at `/404` (en) and `/<locale>/404/` for every other locale; `vercel.json` rewrites missing `/<locale>/…` paths to it.
...
- **Copy:** no user-facing strings in components or views; add a key to `src/copy/en.ts` and every other `src/copy/<locale>.ts`. Structure (paths, ids, URLs) lives in `src/data_files/`, labels in copy keyed by id; keys, not translated labels, drive logic (e.g. `badge: 'hiring'`).
...
- To change a page, edit its view in [src/views/](src/views/) and its strings in [src/copy/](src/copy/); do not duplicate a page into `src/pages/<locale>/`.
